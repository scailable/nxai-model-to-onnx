...
class Darknet(nn.Module):
    def __init__(self, cfgfile, width, height, inference=False, keep_large_objects=None, keep_small_objects=None):
        super(Darknet, self).__init__()

        self.keep_large_objects = keep_large_objects
        self.keep_small_objects = keep_small_objects

        self.inference = inference
        self.training = not self.inference

        self.blocks = parse_cfg(cfgfile)
        self.width = width #int(self.blocks[0]['width'])
        self.height = height #int(self.blocks[0]['height'])
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            return get_region_boxes(out_boxes, self.keep_large_objects, self.keep_small_objects)
