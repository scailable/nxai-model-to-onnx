...
def transform_to_onnx(cfgfile, weightfile, batch_size=1, width=128, height=128, keep_large_objects=True, keep_small_objects=True):
    model = Darknet(cfgfile, width, height, keep_large_objects=keep_large_objects, keep_small_objects=keep_small_objects)
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
        ...
        cfgfile = sys.argv[1]
        weightfile = sys.argv[2]
        large_objects = bool(sys.argv[3])
        small_objects = bool(sys.argv[4])
        transform_to_onnx(cfgfile, weightfile, keep_large_objects=large_objects, keep_small_objects=small_objects)
    else:
