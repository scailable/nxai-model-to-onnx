...
__global__ void gpuYoloLayerV3(float* input, float* output, uint gridSize, uint numOutputClasses,
                               uint numBBoxes)
{
    ...
    int numGridCells = gridSize * gridSize;
    int bbindex = y_id * gridSize + x_id;
...
cudaError_t cudaYoloLayerV3(void* input, void* output, uint& batchSize, uint& gridSize,
                            uint& numOutputClasses, uint& numBBoxes,
                            ...
                            uint64_t outputSize, cudaStream_t stream)
{
    ...
    for (unsigned int batch = 0; batch < batchSize; ++batch)
    {
        gpuYoloLayerV3<<<number_of_blocks, threads_per_block, 0, stream>>>(
            reinterpret_cast<float*>(input) + (batch * outputSize),
